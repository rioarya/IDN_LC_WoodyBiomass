...
df1nu = pd.read_excel('C:\\Work\\Programming\\Practice\\PF_PO_EC_Rel.xlsx', 'PF_PO_S1nu')
df1pl = pd.read_excel('C:\\Work\\Programming\\Practice\\PF_PO_EC_Rel.xlsx', 'PF_PO_S1pl')
df3nu = pd.read_excel('C:\\Work\\Programming\\Practice\\PF_PO_EC_Rel.xlsx', 'PF_PO_Enu')
df3pl = pd.read_excel('C:\\Work\\Programming\\Practice\\PF_PO_EC_Rel.xlsx', 'PF_PO_Epl')
...
dfEnu = pd.read_excel('C:\\Work\\Programming\\Practice\\PF_PO_EC_Rel.xlsx', 'PF_PO_Enu')
dfEpl = pd.read_excel('C:\\Work\\Programming\\Practice\\PF_PO_EC_Rel.xlsx', 'PF_PO_Epl')
...
df = pd.read_excel('C:\\Work\\Programming\\Practice\\PF_PO_EC_Rel.xlsx', 'PF_PO_S1nu')
...
#S1pl
df = pd.read_excel('C:\\Work\\Programming\\Practice\\PF_PO_EC_Rel.xlsx', 'PF_PO_S1pl')
...
#Enu
df = pd.read_excel('C:\\Work\\Programming\\Practice\\PF_PO_EC_Rel.xlsx', 'PF_PO_Enu')
...
#Epl
df = pd.read_excel('C:\\Work\\Programming\\Practice\\PF_PO_EC_Rel.xlsx', 'PF_PO_Epl')
...
Emissions_PF_PO_S1nu = [c_firewood_energy_S1nu, decomp_emissions, TestDSM1nu.o, PH_Emissions_PO_S1nu, PH_Emissions_HWP_S1nu, decomp_tot_CO2_S1nu[:,0]]
Emissions_PF_PO_S1pl = [c_firewood_energy_S1pl, decomp_emissions, TestDSM1pl.o, PH_Emissions_PO_S1pl, PH_Emissions_HWP_S1pl, decomp_tot_CO2_S1pl[:,0]]
...
writer = pd.ExcelWriter('emissions_seq_PF_PO_EC_Rel.xlsx', engine = 'xlsxwriter')
...
##wood-based
#read S1_nucleus
df = pd.read_excel('emissions_seq_PF_PO_EC_Rel.xlsx', 'S1_nucleus') # can also index sheet by name or fetch all sheets
...
#read S1_plasma
df = pd.read_excel('emissions_seq_PF_PO_EC_Rel.xlsx', 'S1_plasma')
...
#read E_nucleus
df = pd.read_excel('emissions_seq_PF_PO_EC_Rel.xlsx', 'E_nucleus') # can also index sheet by name or fetch all sheets
...
#read E_plasma
df = pd.read_excel('emissions_seq_PF_PO_EC_Rel.xlsx', 'E_plasma')
...
#Export to excel
writer = pd.ExcelWriter('GraphResults_PF_PO_EC_Rel.xlsx', engine = 'xlsxwriter')
...
writer = pd.ExcelWriter('C_flows_PF_PO_EC_Rel.xlsx', engine = 'xlsxwriter')
...
Agg_Cflow_PF_PO_S1nu = [c_firewood_energy_S1nu, decomp_emissions, TestDSM1nu.o, OpProcessing_PF_PO_S1nu, Landfill_decomp_PF_PO_S1nu, flat_list_nucleus]
Agg_Cflow_PF_PO_S1pl = [c_firewood_energy_S1pl, decomp_emissions, TestDSM1pl.o, OpProcessing_PF_PO_S1pl, Landfill_decomp_PF_PO_S1pl, flat_list_plasma]
...
#Export to excel
writer = pd.ExcelWriter('AggCFlow_PF_PO_EC_Rel.xlsx', engine = 'xlsxwriter')
...
OC_storage_S1nu = df1nu['Other_C_storage'].values


OC_storage_S1nu = [x/division for x in OC_storage_S1nu]
OC_storage_S1nu = [abs(number) for number in OC_storage_S1nu]

C_LF_S1nu = [x*1/0.82 for x in OC_storage_S1nu]


## define the input flow from the logging/harvesting to wood materials/pellets processing (F2-3)
HWP_S1nu = [x/division for x in df1nu['Input_PF'].values]
HWP_S1nu_energy =  [x*1/3 for x in c_firewood_energy_S1nu]
HWP_S1nu_landfill = [x*1/0.82 for x in OC_storage_S1nu]

HWP_S1nu_sum = [HWP_S1nu, HWP_S1nu_energy, HWP_S1nu_landfill]
HWP_S1nu_sum = [sum(x) for x in zip(*HWP_S1nu_sum)]

#in-use stocks (S-4)
TestDSM1nu.s = [x/division for x in TestDSM1nu.s]
#TestDSM1nu.i = [x/division for x in TestDSM1nu.i]
...
zero_matrix_stocks_S1nu = (tf,1)
stocks_S1nu = np.zeros(zero_matrix_stocks_S1nu)


i = 0
stocks_S1nu[0] = C_LF_S1nu[0] - Landfill_decomp_PF_PO_S1nu[0]

while i < tf-1:
    stocks_S1nu[i+1] = np.array(C_LF_S1nu[i+1] - Landfill_decomp_PF_PO_S1nu[i+1] + stocks_S1nu[i])
    i = i + 1



## calculate aggregate flow of logged wood (F1-2)
HWP_logged_S1nu = [x1+x2 for (x1,x2) in zip(HWP_S1nu_sum, [x*2/3 for x in c_firewood_energy_S1nu])] 


## calculate the stocks in the forest (AGB + undecomposed residue) (S-1a+S-1c)
tf = 201

zero_matrix_ForCstocks_S1nu = (tf,1)
ForCstocks_S1nu = np.zeros(zero_matrix_ForCstocks_S1nu)

i = 0
ForCstocks_S1nu[0] = initAGB - flat_list_nucleus[0] - decomp_emissions[0] - HWP_logged_S1nu[0]

while i < tf-1:
    ForCstocks_S1nu[i+1] = np.array(ForCstocks_S1nu[i] - flat_list_nucleus[i+1] - decomp_emissions[i+1] - HWP_logged_S1nu[i+1])
    i = i + 1


##NonRW materials/energy amount (F9-0-1)
df1nu_amount = pd.read_excel('C:\\Work\\Programming\\Practice\\NonRW_PF_PO_EC.xlsx', 'PF_PO_S1nu')
NonRW_amount_S1nu = df1nu_amount['NonRW_amount'].values

NonRW_amount_S1nu = [x/1000 for x in NonRW_amount_S1nu]



##NonRW emissions (F9-0-2)
emission_NonRW_S1nu = [x/division for x in emission_NonRW_S1nu]
...
dfM_nu = pd.DataFrame.from_dict({'Year':Column1, 
                                    ...
                                    #'F1a-2 (t-C)': PF_PO_S1nu,
                                    #'F1c-2 (t-C)': FP_PO_S1nu,
                                    'F1-2 (t-C)': HWP_logged_S1nu, 
                                    'St-1 (t-C)':ForCstocks_S1nu[:,0], 
                                    'F2-3 (t-C)': HWP_S1nu_sum,  
                                    'F2-6 (t-C)': [x*2/3 for x in c_firewood_energy_S1nu], 
                                    'SM/E (t-C)': [x1-x2-x3 for (x1,x2,x3) in zip(HWP_S1nu_sum, [x*1/0.82 for x in OC_storage_S1nu], [x*1/3 for x in c_firewood_energy_S1nu])],
                                    'F3-5 (t-C)':[x*1/0.82 for x in OC_storage_S1nu],
                                    'F3-6 (t-C)': [x*1/3 for x in c_firewood_energy_S1nu], 
                                   # 'F4-0 (t-C)':,
                                    'St-4 (t-C)': TestDSM1nu.s, 
                                    #'S-4-i (t-C)': TestDSM1nu.i,
                                    'F4-5 (t-C)': TestDSM1nu.o,
                                    'F5-6 (t-C)': TestDSM1nu.o, 
                                    'F5-7 (t-C)': C_LF_S1nu,
                                    'F6-0-1 (t-C)': c_firewood_energy_S1nu,
                                    'F6-0-2 (t-C)': TestDSM1nu.o,
                                    'St-7 (t-C)': stocks_S1nu[:,0],
                                    'F7-0 (t-C)': Landfill_decomp_PF_PO_S1nu,
                                    'F8-0 (t-C)': OpProcessing_PF_PO_S1nu,
                                    'S9-0 (t)': NonRW_amount_S1nu, 
                                    'F9-0 (t-C)': emission_NonRW_S1nu,
                                    })
...
##S1pl
## define the input flow for the landfill (F5-7)
OC_storage_S1pl = df1pl['Other_C_storage'].values


OC_storage_S1pl = [x/division for x in OC_storage_S1pl]
OC_storage_S1pl = [abs(number) for number in OC_storage_S1pl]

C_LF_S1pl = [x*1/0.82 for x in OC_storage_S1pl]


## define the input flow from the logging/harvesting to wood materials/pellets processing (F2-3)
HWP_S1pl = [x/division for x in df1pl['Input_PF'].values]
HWP_S1pl_energy =  [x*1/3 for x in c_firewood_energy_S1pl]
HWP_S1pl_landfill = [x*1/0.82 for x in OC_storage_S1pl]

HWP_S1pl_sum = [HWP_S1pl, HWP_S1pl_energy, HWP_S1pl_landfill]
HWP_S1pl_sum = [sum(x) for x in zip(*HWP_S1pl_sum)]

#in-use stocks (S-4)
TestDSM1pl.s = [x/division for x in TestDSM1pl.s]
#TestDSM1pl.i = [x/division for x in TestDSM1pl.i]
...
zero_matrix_stocks_S1pl = (tf,1)
stocks_S1pl = np.zeros(zero_matrix_stocks_S1pl)


i = 0
stocks_S1pl[0] = C_LF_S1pl[0] - Landfill_decomp_PF_PO_S1pl[0]

while i < tf-1:
    stocks_S1pl[i+1] = np.array(C_LF_S1pl[i+1] - Landfill_decomp_PF_PO_S1pl[i+1] + stocks_S1pl[i])
    i = i + 1



## calculate aggregate flow of logged wood (F1-2)
HWP_logged_S1pl = [x1+x2 for (x1,x2) in zip(HWP_S1pl_sum, [x*2/3 for x in c_firewood_energy_S1pl])] 


## calculate the stocks in the forest (AGB + undecomposed residue) (S-1a+S-1c)
tf = 201

zero_matrix_ForCstocks_S1pl = (tf,1)
ForCstocks_S1pl = np.zeros(zero_matrix_ForCstocks_S1pl)

i = 0
ForCstocks_S1pl[0] = initAGB - flat_list_plasma[0] - decomp_emissions[0] - HWP_logged_S1pl[0]

while i < tf-1:
    ForCstocks_S1pl[i+1] = np.array(ForCstocks_S1pl[i] - flat_list_plasma[i+1] - decomp_emissions[i+1] - HWP_logged_S1pl[i+1])
    i = i + 1


##NonRW materials/energy amount (F9-0-1)
df1pl_amount = pd.read_excel('C:\\Work\\Programming\\Practice\\NonRW_PF_PO_EC.xlsx', 'PF_PO_S1pl')
NonRW_amount_S1pl = df1pl_amount['NonRW_amount'].values

NonRW_amount_S1pl = [x/1000 for x in NonRW_amount_S1pl]



##NonRW emissions (F9-0-2)
emission_NonRW_S1pl = [x/division for x in emission_NonRW_S1pl]
...
dfM_pl = pd.DataFrame.from_dict({'Year':Column1, 
                                    ...
                                    'F1-0 (t-C)': decomp_emissions,
                                    #'F1a-2 (t-C)': PF_PO_S1pl,
                                    #'F1c-2 (t-C)': FP_PO_S1pl,
                                    'F1-2 (t-C)': HWP_logged_S1pl, 
                                    'St-1 (t-C)':ForCstocks_S1pl[:,0], 
                                    'F2-3 (t-C)': HWP_S1pl_sum,  
                                    'F2-6 (t-C)': [x*2/3 for x in c_firewood_energy_S1pl], 
                                    'SM/E (t-C)': [x1-x2-x3 for (x1,x2,x3) in zip(HWP_S1pl_sum, [x*1/0.82 for x in OC_storage_S1pl], [x*1/3 for x in c_firewood_energy_S1pl])],
                                    'F3-5 (t-C)':[x*1/0.82 for x in OC_storage_S1pl],
                                    'F3-6 (t-C)': [x*1/3 for x in c_firewood_energy_S1pl], 
                                   # 'F4-0 (t-C)':,
                                    'St-4 (t-C)': TestDSM1pl.s, 
                                    #'S-4-i (t-C)': TestDSM1pl.i,
                                    'F4-5 (t-C)': TestDSM1pl.o,
                                    'F5-6 (t-C)': TestDSM1pl.o, 
                                    'F5-7 (t-C)': C_LF_S1pl,
                                    'F6-0-1 (t-C)': c_firewood_energy_S1pl,
                                    'F6-0-2 (t-C)': TestDSM1pl.o,
                                    'St-7 (t-C)': stocks_S1pl[:,0],
                                    'F7-0 (t-C)': Landfill_decomp_PF_PO_S1pl,
                                    'F8-0 (t-C)': OpProcessing_PF_PO_S1pl,
                                    'S9-0 (t)': NonRW_amount_S1pl, 
                                    'F9-0 (t-C)': emission_NonRW_S1pl,
                                    })
...
##NonRW materials/energy amount (F9-0-1)
dfEnu_amount = pd.read_excel('C:\\Work\\Programming\\Practice\\NonRW_PF_PO_EC.xlsx', 'PF_PO_Enu')
...
##NonRW materials/energy amount (F9-0-1)
dfEpl_amount = pd.read_excel('C:\\Work\\Programming\\Practice\\NonRW_PF_PO_EC.xlsx', 'PF_PO_Epl')
...
writer = pd.ExcelWriter('C_flows_SysDef_PF_PO_EC_Rel.xlsx', engine = 'xlsxwriter')



dfM_nu.to_excel(writer, sheet_name = 'PF_PO_EC_Mnu', header=True, index=False)
dfM_pl.to_excel(writer, sheet_name = 'PF_PO_EC_Mpl', header=True, index=False)
dfE_nu.to_excel(writer, sheet_name = 'PF_PO_EC_Enu', header=True, index=False)
dfE_pl.to_excel(writer, sheet_name = 'PF_PO_EC_Epl', header=True, index=False)
